(function(ide, cxl) {
	...
	function isProjectFile(projectFiles, path) {
		return projectFiles && projectFiles.find(f => f.filename === path);
	}
	...
	function grepDone(editor, result) {
		var i = 0,
			...
			files = [];
		result = result.split('\n');
		const projectFiles = ide.project.get('files');

		for (; i < result.length; i++) {
			match = GREP_REGEX.exec(result[i]);
			if (match && isProjectFile(projectFiles, match[1])) {
				const desc = match[3];
	...
	ide.plugins.register('grep', {
		commands: {
			grep(term) {
				term =
					term ||
					(ide.editor && ide.editor.token && ide.editor.token.value);
				...
				cxl.ajax({
					...
					progress: function(a) {
						var eol =
							a.target.responseText.lastIndexOf('\n') || a.loaded;
					...
					function(err) {
						editor.$footer.innerHTML =
							'<div class="text-error">Error: ' + err + '</div>';
					}
				);
