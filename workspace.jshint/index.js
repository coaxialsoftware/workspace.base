...
plugin
	.extend({
		...
		findOptions(p, f) {
			var dir = f ? path.dirname(f) : p.path,
				file = dir + '/.jshintrc',
				data;
			if (!fs.existsSync(file) && f) {
				file = p.path + '/.jshintrc';
				if (!fs.existsSync(file)) file = false;
			}
		...
		doLint(request, options) {
			const js = request.features.file.content;
		...
		lintFile(client, data) {
			const options = this.findOptions(data.p, data.f);

			ide.readFile(data.f).then(
				this.doLint.bind(this, client, options, data)
			);
		},
		...
		onMessage(client, data) {
			if (data.op === 'lint')
				this.operation(
					`Linting file ${data.f}`,
					this.lintFile.bind(this, client, data)
				);
		},
